  ...
  <div class="crown-wrapper">
    ...
    <a-form layout="inline" :form="vform" class="query-form" @submit.prevent="getList(true)">
      <div class="query-item-1">
        <div class="quick-picker">
          <DateRange :defaultDate="true" title="提现时间" ref="rg" @current="setCurrent" @change="changeDate"/>
        </div>
    ...
    <div class="page-under-table">
       <ul class="summary-data">
        <li>提现总额：{{summary.totalWithdrawAmount || '0.00'}} 元</li>
        <li>提现人数：{{summary.withdrawUserCount || '0'}} 人</li>
        <li>提现次数：{{summary.withdrawCount || '0'}} 次</li>
        <li>&nbsp;</li>
      </ul>
    </div>
...
export default class DepositQuery extends Vue {
  table = {
    ...
    columns: [
      ...
      { title: "提现金额",  dataIndex: 'withdrawAmount', sorter: true},
      { title: "提现时间", dataIndex: 'withdrawTime', sorter: true}
    ],
    dataSource: [],
  ...
  changePagination(pagination, filters, sorter) {
    ...
    this.init.query.sortList = [
      {
        sortField:  Object.keys(sorter).length === 0 ?  'withdrawAmount' : sorter?.field === "withdrawTime" ? "handleTime" : sorter?.field,
  ...
  async getList(refresh) {
    ...
    const res = await this.$service.fetch('/lottery-api/withdrawRecord/queryPageWithdrawRecordByDirect', values);
  ...
  async getSummary(params) {
    const res = await this.$service.fetch('/lottery-api/withdrawRecord/queryWithdrawRecordSummaryByDirect', params);
    if (res.code !== 1) return res.message && this.$message.error(res.message);
