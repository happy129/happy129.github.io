  ...
  <section class="deposit-wrapper">
    <a-form layout="inline">
      ...
      <div class="mt15">
        ...
        <a-button :loading="loading" type='primary' @click="_signRecordQueryByPage(true)">查询</a-button>
			</div>
        </a-form-item>
      </div>
    </a-form>
...
export default class DepositTotal extends Vue {
  ...
  table = {
    ...
    columns: [
      ...
      {title: '统计日期', dataIndex: 'signDate'},
      {title: '有效投注', dataIndex: 'validBetAmount'},
      {title: '签到金额', dataIndex: 'receiveAmount'},
      {title: '签到时间', dataIndex: 'createTime'},
    ]
  }
  // 弹窗里面用到的数据
	defaultValueTime=[];
...
// 切换分页 
  ...
  resetPagination() {
	...
	/* 获取首充列表 */
	async _signRecordQueryByPage(refresh){
      ...
      this.loading = true;
      let { data: req } =	await Api.signRecordQueryByPage(this.form);
      this.loading = false;
