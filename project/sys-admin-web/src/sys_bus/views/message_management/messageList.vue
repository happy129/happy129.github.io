  ...
  <div class="page-container">
    <div class="mt15">
      <a-form layout="inline" :form="form">
        ...
        <span style="margin-left:10px;margin-top:3px;display:inline-block;">
          <a-button type="primary" ref="btnSave" v-permission="20351" @click="_query">查询</a-button>
    ...
    <div class="mt15">
      <a-table :rowSelection="{selectedRowKeys: table.selectedRowKeys, onChange: onSelectChange}" :columns="table.columns" :dataSource="table.tableData" :pagination="false" :rowKey="e => e.id" class="mytable" bordered >
        ...
        <template slot="operate" slot-scope="r, text">
          <a-button type="primary" size="small" v-permission="203541" @click="_editAndAdd(text)">查看</a-button >
          <!-- <a-button type="primary" size="small" @click="_removeMessage(text)">删除</a-button > -->
        </template>
      </a-table>
      <div style="text-align:right;padding-top:10px;display:flex;justify-content:space-between;">
				<div></div>
				<!-- <a-button type="primary" ref="btnSave" @click="_batchRemoveMessage">批量删除</a-button> -->
        <a-pagination :current="pagination.pageNo" :total="pagination.total" @change="changePagination" />
      </div>
    </div>
    <a-modal title="查看消息" v-model="modalShow" :footer="null" :destroyOnClose="true" :maskClosable="false" >
      <div style="text-align:left;">
        <a-form :form="form">
          ...
          <!-- <a-form-item  label="指定商户" v-bind="formItemLayout">
						<a-select  :disabled="Object.keys(details).length?true:false" mode="multiple" v-decorator="['list',{initialValue: details.list,rules: [{required: true, message: '请选择商户' }] }]" placeholder="请选择商户">
          ...
          <a-form-item  label="消息类别" v-bind="formItemLayout">
           ...
           <a-select :disabled="Object.keys(details).length?true:false" placeholder="选择消息类别" v-decorator="['createTime', { initialValue: details.createTime,rules: [{ required: true, message: '请选择消息类别' }] }]">
            <a-select-option v-for="(item, index) in table.queryAllMessageTypeList" :key="index" :value="item.id">{{item.typeName}}</a-select-option>
          </a-select>
          </a-form-item>
