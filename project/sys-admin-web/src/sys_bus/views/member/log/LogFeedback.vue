     ...
     <a-form layout="inline" :form="vform">
        ...
        <a-form-item label="问题">
            <a-select v-decorator="['problemType', { initialValue: '' }]" :dropdownMatchSelectWidth="false" @change="changeSelection">
              <a-select-option value="">全部</a-select-option>
              <a-select-option v-for="(item, index) in dictionary.userFeedbackType"  :key="index" :value="item.value">{{item.name}}</a-select-option>
            </a-select>
        </a-form-item>
        <a-form-item style="display:none"><a-input v-decorator="['feedbackType', {initialValue: 1}]"/></a-form-item>
        <a-form-item label="会员账号"><a-input v-decorator="['userName', { initialValue: ''}]"/></a-form-item>
        <a-form-item label="用户ID"> <a-input type="number" @change="changeInput" v-decorator="['userId', { initialValue: ''}]"/></a-form-item>
    ...
    <div class="page-under-table">
        <a-table class="mytable" :loading="init.loading" bordered :columns="table.columns" :dataSource="table.dataSource" :pagination="table.pagination"  @change="changePagination" :rowKey="e => e.id">
          ...
          <template slot="problemType" slot-scope="text, records">
            <span>{{getTextByDictionary(records.problemType, dictionary.userFeedbackType)}}</span>
          </template>
          ...
          <template slot="operation" slot-scope="text, records">
            <a-button type="primary" size="small" v-permission="20028"  @click="doModify(records)">{{records.status == 0 ? '回复': '详情'}}</a-button>
          </template>
        </a-table>
    </div>
...
@Component({ dictionary: "clientType,userFeedbackType", formOptions: "id"})
