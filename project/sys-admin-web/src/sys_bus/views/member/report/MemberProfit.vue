...
export default class MemberProfit extends Vue {
  table = {
    columns: [
      { title: "排行", scopedSlots: { customRender: "index" } },
      { title: "会员账号", dataIndex: "userName" },
      { title: "余额", dataIndex: "balance", sorter: true  },
      { title: "盈亏", dataIndex: "totalProfit", sorter: true  },
      { title: "有效投注", dataIndex: "validOrderAmount", sorter: true  },
      { title: "中奖金额", dataIndex: "totalWinAmount", sorter: true  },
  ...
  changeTabSort(pagination, filters, sorter) {
    ...
    // 点击排序
     if (sorter.order) {
      ...
      sortParams.sortField = "totalProfit";
      sortParams.sortOrder = "desc"
    }
  ...
  async getList(refresh,  page=true) {
    ...
    const res = await this.$service.fetch('/lottery-admin-api/user/profitRankings', params);
