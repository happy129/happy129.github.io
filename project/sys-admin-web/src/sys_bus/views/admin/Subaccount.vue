  ...
  <div class="subaccount-box page-container">
    <a-form layout="inline">
      <a-form-item label="用户名：">
        ...
        <a-button type="primary" @click="add" class="newB">新增</a-button>
      </a-form-item>
    </a-form>
    <br>
    ...
    <!-- isBuiltIn区分admin -->
      <template slot="operate" slot-scope="r, text">
        <a-button type="primary" size="small" @click="getDetail(text.id, text.isBuiltIn)">编辑</a-button>
        <a-button type="primary" size="small" @click="ePwd(text)">修改密码</a-button>
        <a-popconfirm title="确定删除吗?" @confirm="deleteAdmin(text)" @cancel="void(0)" okText="确定" cancelText="取消" >
          <a-button v-if="text.isBuiltIn===0" type="primary" size="small">删除</a-button>
        </a-popconfirm>
        <a-popconfirm title="确定冻结吗?" @confirm="editStatusAdmin(text, 0)" @cancel="void(0)" okText="确定" cancelText="取消" v-if="text.isEnabled===1" >
          <a-button v-if="text.isBuiltIn===0" type="primary" size="small">冻结</a-button>
        </a-popconfirm>
    ...
    <addAdmin v-if="showAddAdmin" @close="showAddAdmin=false" @ok="searchs"/>
    <editAdmin :admin="isAdmin" v-if="showEditAdmin" @close="showEditAdmin=false" :detail="detail" @ok="searchs"/>
    <editPassword v-if="showEditPassword" @close="showEditPassword=false" @ok="searchs" :id="currentid" :userName="currentUserName" :nickName="currentNickName" />
  </div>
</template>
<script>
import Vue from "vue";
