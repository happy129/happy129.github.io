      ...
      <a-table 
        ...
        >
          ...
          <template slot="operate" slot-scope="text,records">
            <a-button v-permission="20096" type="primary" size="small" @click="onSave(records)">保存</a-button>
          </template>
      </a-table>  
  </div>
</template>
...
@Component
export default class PayoutManage extends Vue {
  table = {
    key: 1,
    columns: [
  ...
  async getList() {
    ...
    // paymentType=4
    const res = await this.$service.fetch('/config-admin-api/thirdPayment/queryPageThirdGateway', Object.assign({ paymentType: 20 }, this.init.query));
