<template>
    <section class="finance-check-container">
        ...
        <div class="page-under-table finance-wrapper">
            ...
            <a-table class="mytable mt10"  bordered :columns="table.columns" :dataSource="table.dataSource" :pagination="table.pagination" :rowKey="(e, i) => i" @change="changePagination">
                <template slot="userName"><p>支付方式</p> <p>银行卡号</p> </template>
                <template slot="paymentName" slot-scope="text, records">
                    <p>{{records.paymentName}}</p>
                    <p>{{records.bankCardNo}}</p>
                </template>
                <template slot="userName2"><p>银行卡名称</p> <p>持卡人</p> </template>
                <template slot="bankName" slot-scope="text, records">
                    <p>{{records.bankName}}</p>
                    <p>{{records.realName}}</p>
                </template>
                <template slot="successRate" slot-scope="t, r">
                    <span>
                        {{$tools.toFixed(r.successRate*100, 2, false)}}%
                    </span>
                </template>
            </a-table>
        </div>
    ...
    export default class FinanceCheck extends Vue {
        table = {
            ...
            columns: [
                ...
                // { title: "银行卡号", dataIndex: "bankCardNo", align: "left" },
                { dataIndex: "name1", scopedSlots: { customRender: "paymentName", title: "userName" }, width: 200 },
                { dataIndex: "name2", scopedSlots: { customRender: "bankName", title: "userName2" } },
                // { title: "银行卡名称", dataIndex: "bankName", align: "left" },
        ...
        init = {
            ...
            allUserGroup: [],
            // 查询参数
            query: { pageNo: 1, pageSize: 20, handleTimeStart: "", handleTimeEnd: "", paymentType: 1, thirdPaymentId: 0 },
