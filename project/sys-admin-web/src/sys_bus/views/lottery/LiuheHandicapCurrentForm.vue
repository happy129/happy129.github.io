  ...
  <div class="wrapper">
    <a-form :form="vform" layout="inline">
      ...
      <a-form-item label="期号" >
		  <!-- ['2','3','4'].includes(vform.data.status) -->
        <a-input :disabled="true" v-decorator="['issueNo']"/>
      </a-form-item>
      <a-form-item label="状态" v-if="vform.data.status">
		  <!-- v-if="3 == vform.data.status" -->
        <template>
          {{dictionary.marksixStatus.get(vform.data.status)}}
        </template>
		<!--  @click.native="toChangeStatus()" -->
        <!-- <a-button ref="btnChangeStatus" v-else-if="vform.data.id" size="small" ghost type="primary">{{dictionary.marksixStatus.get(vform.data.status)}}</a-button> -->
      </a-form-item>
      <br>
      ...
      <a-form-item label="自动封单时间">
		  <!-- :disabled="['3','4'].includes(vform.data.status)" -->
    ...
    <!-- <div style="width: 790px; text-align: center;">
      <a-button type="primary" ref="btnSave" :disabled="['3','4'].includes(vform.data.status)" @click.native="toSubmit()">保存设置</a-button>
    </div> -->
  </div>
</template>
