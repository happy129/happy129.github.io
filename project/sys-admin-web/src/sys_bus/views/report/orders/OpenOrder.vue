	...
	<section class="report-order">
		<a-form :form="vform" class="report-form1" @submit.prevent="getList(true)">
			<!-- <a-form-item label="撤销平台">
				<a-select v-decorator="['platformId']" style="width:120px;">
					<a-select-option value="">1号平台</a-select-option>
				</a-select>
			</a-form-item> -->
			...
			<div>
				<a-row :gutter="8">
					...
					<a-col :span="7">
						 <a-button v-permission="20168" @click="openCancelOrder">开放撤单</a-button>
					</a-col>
				</a-row>
			</div>
		</a-form>
		...
		<div class="page-under-table">
			<a-table class="mytable" bordered :columns="table.columns" :rowKey="(e,i) => i" :loading="init.loading" :dataSource="table.dataSource" :pagination="false">
				<template slot="status" slot-scope="text, record">
					<span :class="[record.status == 0 ? 'g-red' : 'g-green']">{{ record.status == 0 ? "撤单已关闭" : "撤单开放中" }}</span>
				</template>
				<template slot="operation" slot-scope="text, record">
					<a-button type="primary" v-permission="201682" @click="closeCancelOrder(record.id)" v-if="record.status != 0" size="small">关闭撤单</a-button>
					<a-button type="primary" v-permission="201681" size="small" @click="openOrder(record.id)" v-else>开放撤单</a-button>
				</template>
			</a-table>
