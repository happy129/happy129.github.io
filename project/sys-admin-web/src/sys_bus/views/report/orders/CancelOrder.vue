<template>
	...
	<section class="report-order">
		<a-form :form="vform" layout="inline" @submit.prevent="getList(true)">
			...
			<div>
				<a-button v-permission="20166" type="primary" @click="cancelOrder">撤销注单</a-button>
			</div>
		</a-form>

		<div class="page-under-table">
			<a-table class="mytable" bordered :columns="table.columns" :pagination="false"  :rowKey="(e,i) => i" :loading="init.loading" :dataSource="table.dataSource">
...
export default class CancelOrder extends Vue {
  ...
  async loadLotteryData(selectedOptions) {
	...
	async getList(refresh) {
		...
		// 后台不需要传该字段，进行删除操作
		delete query.lotteryCategoryId;
