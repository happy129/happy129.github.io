<template>
  <div class="report-wrapper">
    <a-table class="my-table" :columns="table.columns" :dataSource="table.dataSource" :pagination="false" :rowKey="(e, i) => i">
      <template slot="totalProfit" slot-scope="text, record">
         <span :class="[record.totalProfit > 0 ? 'g-green' : 'g-red' ]"> {{  record.totalProfit }} </span>
      </template>
      <template slot="profitRate" slot-scope="text, record">
        <span :class="[record.profitRate > 0 ? 'g-green' : 'g-red' ]"> {{  record.profitRate }}  %</span>
      </template>
      <template slot="expandedRowRender" slot-scope="params">
         <a-table v-if="params.normalSummary.length > 0" :columns="table.columns2" :dataSource="params.normalSummary" class="table-item1"  :rowKey="(e, i) => `${i}-${Math.random()}`"
...
export default class LotteryGame extends Vue {
  ...
  table = {
    columns: [
      ...
      { title: "平台盈亏", dataIndex: "totalProfit" , scopedSlots: { customRender: 'totalProfit' }, width: '100px'},
      { title: "盈利率", dataIndex: "profitRate", scopedSlots: { customRender: 'profitRate' }, width: '120px'}
    ],
    dataSource: [],
    innerDataSource: [],
  ...
  changePagination(pagination) {
    ...
    if (res.code !== 1) return res.message && this.$message.error(res.message);
      ...
      //         item.normalSummary[elem] = parseFloat(item.normalSummary[elem]).toFixed(3);
      //       }
      //       if (typeof item.chaseSummary === 'object' && item.chaseSummary !== null) {
      //         item.chaseSummary[elem] = parseFloat(item.chaseSummary[elem]).toFixed(3);
      //       }
      //       if (typeof item.lotterySummary === 'object' && item.lotterySummary !== null) {
      //         item.lotterySummary[elem] = parseFloat(item.lotterySummary[elem]).toFixed(3);
      //       }
      //     });
      // });
