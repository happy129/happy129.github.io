  ...
  <div class="page-container">
    ...
    <div class="mt15">
      <a-form layout="inline">
        ...
        <div
          ...
          v-if="!['KY', 'TH'].includes(params.current)"
        >
          ...
          <a-form-item label="局号">
            <a-input
              placeholder="局号"
              type="text"
              v-model="params.public.gameCode"
            />
          </a-form-item>
        </div>
    ...
    <div class="mt15">
      <a-table
        ...
        bordered
      >
        <span slot="oderStatus" slot-scope="r, text">{{
          dictionary.agOrderStatus.get(text.status + "")
        }}</span>
        <!-- <template slot="option" slot-scope="r, row">
          <a-button type="primary" size="small" @click="_boardDetails(row)">牌局详情</a-button>
        </template> -->
      </a-table>
      <div style="display:flex;justify-content:space-between;margin-top:10px;">
        <strong
          >总下注金额：{{ totalTranferData.totalBetAmount }} 总派彩金额：{{
            totalTranferData.totalBonus
          }}
          总有效投注：{{ totalTranferData.totalValidBetAmount }}</strong
        >
        <a-pagination
    ...
    >
      <a-table
        ...
        bordered
      >
        <template slot="gameId" slot-scope="r">
          <p v-for="(item, i) in r" :key="i">
            {{ item.position }}：{{ item.card.join() }}
          </p>
        </template>
      </a-table>
    </a-modal>
...
export default class BetRecord extends Vue {
  ...
  boards={
    ...
    data:[],
    columns:[
      { title: "牌局信息", dataIndex: "gameId",scopedSlots: {customRender: "gameId"}  },
      { title: "玩家座位", dataIndex: "chairId"},
		]
  };
  ...
  table = {
    ...
    KY: [
      ...
      { title: "操作", dataIndex: "option",scopedSlots: {customRender: "option"} }
		],
		TH: [
      { title: "商户平台", dataIndex: "platformName" },
      { title: "会员账号", dataIndex: "userName" },
	...
	tabClick(tab){
		...
		this.params[this.params.current].platformId=this.platformId;
	...
	/** 获取牌局详情 */
  async _boardDetails(row) {
    // this.boards.cardValue=row.cardValue;
    // console.log(row.cardValue,row.kindId)
    // this.boardvisible=true;
    // this.boards.data=[{
    //   gameId:Boars.ConversionCard(row),
    //   chairId:row.chairId
    // }]
	}
	/** 翻页 */
  ...
  async _queryThAgKyirdKyOrderSummary(value,url) {
	...
	/** 根据查询条件查询 */
  async _query() {
		if(this.params.public.startTime.indexOf('Invalid')!==-1){ this.params.public.startTime=""; this.params.public.endTime=""; }
		this.pagination.pageNo = 1;
		this.params.public.platformId=this.platformId;
