  ...
  <div class="page-container">
    ...
    <div class="mt15">
      <a-form layout="inline" :form="form">
        ...
        >
          ...
          >
            <a-select-option
              v-for="(v, i) of merchantPlatformList"
              :value="v.id"
              :key="i"
              >{{ v.platformName }}</a-select-option
            >
          </a-select>
        </a-form-item>
    ...
    <div class="mt15">
      ...
      >
        ...
        <template slot="operate" slot-scope="r, row">
          <a-button type="primary" size="small" @click="_verify(row)"
            >核实</a-button
          >
          <template v-if="row.status === 3">
            <a-button type="primary" size="small" @click="_sucessOrFail(row, 1)"
              >成功</a-button
            >
            <a-button type="primary" size="small" @click="_sucessOrFail(row, 0)"
              >失败</a-button
            >
          </template>
        </template>
...
export default class ChangeRecord extends Vue {
  ...
  //   if (res?.data?.body?.code !== 1) return;
	...
	// 	this.params.platformId=this.merchantPlatformList[0].id;
	// 	this.$refs.xtabs.changeTab();
  // }
  //查询平台接口
	async _getMerchantPlatformList() {
	// 请求参数，不用管api上的header
  ...
  let res = await Api.queryAllByList();
	...
	/** 核实功能 */
	async _verify(text){
    let res = await this.$service.fetch("/lottery-admin-api/thirdTransferRecord/verifyTransferRecord",{
    id:text.id,
		platformId: this.params.platformId,
		thirdGameCode: this.params.thirdGameCode,
	...
	/** 轮询未知时显示失败成功功能 */
	async _sucessOrFail(row,sn){
    let res = await this.$service.fetch("/lottery-admin-api/thirdTransferRecord/manualHandleTransferRecord",{
    id:row.id,
    platformId: this.params.platformId,
    status:sn,
		thirdGameCode: this.params.thirdGameCode,
