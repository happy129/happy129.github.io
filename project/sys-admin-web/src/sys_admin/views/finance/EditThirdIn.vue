      ...
      <div style="padding:20px;text-align:left;">
        <a-form layout="inline" :form="form">
          <a-form-item label="第三方支付">
            <a-input
              v-decorator="['paymentChannelName', {initialValue:details.paymentChannelName, rules: [{ required: true }]} ]"
            ></a-input>
          </a-form-item>
          <a-form-item label="总控开关">
            <a-select
              style="width:120px;"
              v-decorator="[ 'isEnabledForcedly', {initialValue:details.isEnabledForcedly, rules: [{ required: true }]} ]"
            >
          ...
          <a-form-item label="第三方编码">
            <a-input style="width:400px;" v-decorator="[ 'module', {initialValue: details.module, rules: [{ required: true, pattern: /^[.\r\n\s\S\w\W\D\d]{0,10}$/i, message: '第三方编码长度不能超过10'}]}]"></a-input>
          </a-form-item>
          ...
          <a-form-item label="接入方式">
            <a-select
              style="width:120px;"
              v-decorator="[ 'encodeType', {initialValue: details.encodeType, required: true }]"
            >
          ...
          <a-form-item label="对接备注">
            <a-input style="width:400px;" v-decorator="['remark', {initialValue: details.remark, rules: [{ required: true,  pattern: /^[.\r\n\s\S\w\W\D\d]{0,200}$/i, message: '长度不能超过200'}] }]"></a-input>
          </a-form-item>
          ...
          <div v-for="(v, i) of lists" :key="i">
            <a-form-item class="mt5">
              ...
              <a-input style="width:171px;margin-right:20px;" placeholder="字段名称" v-model="v.columnDesc" @blur="testcolumnDesc"/>

              <a-input style="width:171px;" placeholder="字段标识" v-model="v.columnName" @blur="testcolumnName"/>
...
<script>
import Vue from "vue";
import { Component, Lifecycle, InjectComponents, State, Prop, Watch, Computed } from "vue-decorators";
import * as Api from "@api/admin";

@Component
export default class EditThirdIn extends Vue {
  lists = [];
  loading = false;
  iserror = false;
