          ...
          <div v-for="(v, i) of lists" :key="i">
            <a-form-item class="mt5">
              <a-icon
                ...
                @click="removeRow(v)"
              />
              <a-input style="width:171px;margin-right:20px;" v-model="v.columnName" placeholder="字段名称"></a-input>
              <a-input style="width:171px;" v-model="v.columnDesc" placeholder="字段标识"></a-input>
            </a-form-item>
          </div>
        </a-form>
      </div>
...
@Component
export default class AddThirdOut extends Vue {
  lists = [];
  loading = false;
  isProcess = false;
  iserror = false;
  ...
  submit() {
    ...
    if (this.iserror) {
      ...
      this.form.validateFields(async (e, v) => {
        ...
        //   if (item.name === v.name) {
        //     this.$message.error("第三方支付名称已存在，不能重复添加");
        //     this.isProcess = true;
        //   } else {
        ...
        let r = await this.$service.fetch(
                "/config-admin-api/thirdWithdrawal/addThirdWithdrawal",
                { thirdWithdrawalConfig: this.lists, ...v }
        );
        this.loading = false;
