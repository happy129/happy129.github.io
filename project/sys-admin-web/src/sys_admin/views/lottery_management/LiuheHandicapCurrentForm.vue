  ...
  <div class="wrapper">
    <a-form :form="vform" layout="inline">
      ...
      <a-form-item label="期号" >
        <a-input :disabled="['2','3','4'].includes(vform.data.status)" v-decorator="['issueNo']"/>
      </a-form-item>
      <a-form-item label="状态" v-if="vform.data.status">
        <template v-if="3 == vform.data.status">
          {{dictionary.marksixStatus.get(vform.data.status)}}
        </template>
        <a-button ref="btnChangeStatus" @click.native="toChangeStatus()" v-else-if="vform.data.id" size="small" ghost type="primary">{{dictionary.marksixStatus.get(vform.data.status)}}</a-button>
      </a-form-item>
      <br>
      <a-form-item label="自动开盘时间">
        <a-date-picker :disabled="['2','3','4'].includes(vform.data.status)" showTime format="YYYY-MM-DD HH:mm:ss" placeholder="请选择" v-decorator="['beginTime', $rules.requiredSelect()]"/>
      </a-form-item>
      <a-form-item label="自动封单时间">
        <a-date-picker :disabled="['3','4'].includes(vform.data.status)" showTime format="YYYY-MM-DD HH:mm:ss" placeholder="请选择" v-decorator="['endTime', $rules.requiredSelect()]"/>
      </a-form-item>
    </a-form>
    <div style="width: 790px; text-align: center;">
      <a-button type="primary" ref="btnSave" :disabled="['3','4'].includes(vform.data.status)" @click.native="toSubmit()" v-permission="10045">保存设置</a-button>
    </div>
  </div>
</template>
...
export default class LiuheHandicapCurrentForm extends Vue {
  ...
  vformdata = {};
  marksixStatus={ 0:"预开盘", 1:"等待开盘中", 2:"开盘中", 3:"封盘中", 4:"已开奖" };


	/*   1.当期盘口 如果是等待开盘状态 仅能修改时间
		2.当期盘口 如果是开盘中   仅能修改封单时间
		3.当期盘口 如果是封盘中   所有字段无法修改*/
...
//   }
  ...
  async fetchCurrentCap() {
    ...
    const res = await this.$service.fetch('/config-admin-api/marksixIssue/getCurrentIssueInfo', {});
