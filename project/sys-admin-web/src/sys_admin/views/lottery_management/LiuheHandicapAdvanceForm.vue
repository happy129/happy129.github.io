    ...
    <a-form :form="vform" layout="inline">
      ...
      <a-form-item label="状态" v-if="vform.data.status">
        <a-button
          ...
          ghost
          type="primary"
        >{{dictionary.marksixStatus.get(vform.data.status)}}</a-button>
      </a-form-item>
      <br>
      <a-form-item label="自动开盘时间">
        <a-date-picker
          ...
          placeholder="请选择"
          v-decorator="['beginTime', $rules.requiredSelect()]"
        />
      </a-form-item>
      <a-form-item label="自动封单时间">
        <a-date-picker
          ...
          format="YYYY-MM-DD HH:mm:ss"
          placeholder="请选择"
          v-decorator="['endTime', $rules.requiredSelect()]"
        />
      </a-form-item>
    </a-form>
...
export default class LiuheHandicapCurrentForm extends Vue {
  ...
  async fetchCurrentCap() {
    ...
    const res = await this.$service.fetch(
      "/config-admin-api/marksixIssue/getNextIssueInfo",
      {}
    );
  ...
  async toChangeStatus() {
    ...
    status = +status + 1;
    const res = await this.$service.fetch(
      "/config-admin-api/marksixIssue/editStatus",
      { status, id }
    );
    if (1 != res.code) return res.message && this.$message.error(res.message);
