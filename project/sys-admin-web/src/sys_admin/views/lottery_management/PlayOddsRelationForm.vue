...
<template>
  <a-modal class="lottery_category_form" v-model="visible" :maskClosable="false" 
    ...
    <a-form :form="vform">
      ...
      <a-form-item label="关联奖项" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
        <a-select v-decorator="['bindId']">
          <a-select-option v-for="(item,index) in params.relationItems" :key="index" :value="`${item.id}`">{{item.awardItem}}</a-select-option>
        </a-select>
      </a-form-item>
...
export default class PlayOddsForm extends Vue {
  ...
  async submit() {
    ...
    this.$emit('close', form.values);
  }

  @Lifecycle created() {
    // 设定初始值
    const bindId =  this.params.relationItems[0]?.id || '';
    this.vform.setValue('bindId', bindId);
  }
}
</script>
