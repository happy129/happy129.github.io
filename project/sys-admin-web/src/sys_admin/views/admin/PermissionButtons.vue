...
export default class PermissionButtons extends Vue {
  ...
  async _getAllPermissionList(){
		let res = await this.$service.fetch("/config-admin-api/config/permission/queryAllStructuredAvailablePermission",{ systemType: 0});
  ...
  async save() {
    ...
    this.permissionList=[];
    await this.formatPermissionAndInit();
  }
  /** 勾选或不勾选按钮权限处理方法 */
