  ...
  <div class="page-container">
    ...
    <a-form class="mt10" layout="inline">
      ...
      <a-form-item label="选择模块" style="margin-left:20px;">
        <a-select v-model="moduleType" style="width:140px">
          ...
          <a-select-option v-for="(item, index) in dictionary.systemModule" :key="index" :value="item.value">{{item.name}}</a-select-option>
        </a-select>
      </a-form-item>
      ...
      <a-form-item>
        <a-button type="primary" @click="submit" style="margin-left:20px;" ref="btnSave">查询</a-button>
      </a-form-item>
    </a-form>
    ...
    <a-table :columns="columns" :dataSource="tableData" :pagination="pagination" @change="changePagination" :rowKey="e=>e.id" class="mytable" bordered>
      <template slot="moduleType" slot-scope="text, record">
        {{ $getText(record.moduleType,  dictionary.systemModule) }}
      </template>
    </a-table>
